...
class OLT:
    ...
    def __init__(self, env, splitter_in, splitter_out):
        ...
        self.rnn_model = self.load_rnn_model("ruta/al/modelo_entrenado.h5")  # Ruta al archivo del modelo entrenado
        ...
        self.first_iteration = True  # Variable para rastrear la primera iteración
        self.assign_B_max(B_AVAILABLE)  # Llamada a la función para asignar B_max

    def load_rnn_model(self, model_path):
        return tf.keras.models.load_model(model_path)

    def predict_B_max(self, B_AVAILABLE):
        input_data = np.array([[B_AVAILABLE, self.w_sla[i]] for i in range(len(self.w_sla))])
        predicted_B_max = self.rnn_model.predict(input_data)
        return predicted_B_max

    def assign_B_max_initial(self, B_AVAILABLE):
        B_max_initial = [B_AVAILABLE*self.w_sla[i]/sum(self.w_sla) for i in range(len(self.w_sla))]
        return B_max_initial

    def assign_B_max_with_rnn(self, B_AVAILABLE):
        predicted_B_max = self.predict_B_max(B_AVAILABLE)
        return predicted_B_max

    def assign_B_max(self, B_AVAILABLE):
        if self.first_iteration:
            self.B_max = self.assign_B_max_initial(B_AVAILABLE)
            self.first_iteration = False
        else:
            predicted_B_max = self.assign_B_max_with_rnn(B_AVAILABLE)
            for i in range(len(predicted_B_max)):
                self.B_max.append(predicted_B_max[i])
        ...
        for i in range(N_ONTS):
            ...
            #self.B_max.append((B_AVAILABLE*self.w_sla[i])/sum(self.w_sla)+(random.randrange(-8, 8) * 1000))
            # watch
            self.B_guaranteed.append(138000)
 ...
 ## Aqui es donde se hace extracción de datos
        ...
        carpeta_resultados = 'fijo_resultados_csv_L08_Bmax'
