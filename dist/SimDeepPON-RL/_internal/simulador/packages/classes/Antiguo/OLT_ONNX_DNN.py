...
def consultar_B_max(onnx_model, B_demand_bits, Onu_id, Carga):
    # Crear una sesión de ONNX Runtime
    session = onnxruntime.InferenceSession(onnx_model)

    # Preparar datos de entrada como un arreglo de numpy
    B_demand_bits_scalar = B_demand_bits[0]
    Onu_id_scalar = Onu_id[0]
    Carga_scalar = Carga[0]
    input_data = np.array([[B_demand_bits_scalar, Onu_id_scalar, Carga_scalar]], dtype=np.float32)

    # Ejecutar el modelo
    output = session.run(None, input_data)[0]

    # Retornar el valor predicho de B_max
    return output[0]
...
class OLT:
    ...
    def procesa_report(self, env, mensaje_report):
        ...
        # Realizar consulta al modelo para estimar B_max
        # Consultamos el modelo para obtener B_max
        B_demand_bits = np.array([self.B_demand[ont_id]], dtype=np.float32)
        Onu_id = np.array([ont_id], dtype=np.float32)
        Carga = np.array([0.7], dtype=np.float32)  # Aquí debes ajustar el valor de carga según corresponda
        self.predicted_B_max = consultar_B_max('modelo_entrenado_DNN.onnx', B_demand_bits, Onu_id, Carga)
        print(self.predicted_B_max)
        
        # Actualizamos el ancho de banda que a cada ONU se le permite transmitir, según IPACT
        
        # Actualizamos el ancho de banda máximo estimado
        ...
        #Aqui tiene que hacerse alguna "predicción" sobre B_max, dado B_demand
        ...
        #self.B_alloc[ont_id] = min(self.B_demand[ont_id], self.predicted_B_max[ont_id]) + tamano_report       
...
modelo_ONNX = 'modelo_entrenado_DNN.onnx'  # Reemplaza con la ruta correcta del archivo ONNX
#olt_instancia = OLT(modelo_ONNX)