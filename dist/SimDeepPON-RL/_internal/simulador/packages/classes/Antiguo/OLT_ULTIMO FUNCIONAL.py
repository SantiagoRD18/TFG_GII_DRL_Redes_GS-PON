...
class OLT:
    ...
    def __init__(self, env, splitter_in, splitter_out):
        ...
        for i in range(N_ONTS):
            ...
            #self.B_max.append((B_AVAILABLE*self.w_sla[i]+(random.randrange(1, 6) * 1000))/sum(self.w_sla))
            #self.B_max.append((B_AVAILABLE*self.w_sla[i])/sum(self.w_sla)+(random.randrange(-6, 6) * 10000))
            # watch
            ...
            self.B_alloc_acum_MBS.append(self.B_alloc_acum[i])
            
            
            #print(self.B_alloc_acum_MBS)
    ...
    def procesa_report(self, env, mensaje_report):
       ...
       # Actualizamos el tiempo de transmisión que a cada onu se le permite transmitir, según IPACT
        self.T_alloc[ont_id] = self.B_alloc[ont_id]/R_tx #Aqui es donde se hace asignacion por defecto si no hay naipe.
        self.T_alloc_acum[ont_id] += self.T_alloc[ont_id] 
        

        print(self.T_alloc_acum[ont_id])
 ...
 ## Aqui es donde se hace extracción de datos
        ...
        carpeta_resultados = 'resultados_csv_L1_Bmax_RND'
