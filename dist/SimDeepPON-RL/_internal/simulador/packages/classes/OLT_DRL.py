...
from simulador.packages.classes.MensajeGate import MensajeGate
from simulador.packages.classes.MensajeReport import MensajeReport
...
from simulador.packages.classes.InterfazAgente import AgentInterface
...
class OLT:
    # Simula la OLT
    def __init__(self, env, splitter_in, splitter_out, fichero_modelo):
        self.env = env
        ...
        self.interface = AgentInterface(N_ONTS, R_tx, T_CICLO, self.B_guaranteed, self.B_Disp, self.B_max, L_BUFFER_ONTS, fichero_modelo)
    ...
    def procesa_report(self, env, mensaje_report):
        ...
        self.B_alloc[ont_id] = self.interface.predict_values(self.B_demand, ont_id)
        self.B_alloc[ont_id] = min(self.B_alloc[ont_id], self.B_demand[ont_id]) + tamano_report
